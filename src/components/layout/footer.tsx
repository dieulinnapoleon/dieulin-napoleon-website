...
export function Footer() {
  ...
  useEffect(() => {
    fetch('/api/social-links')
      .then((r) => r.json())
      .then((data) => { if (data.links?.length) setSocials(data.links); })
      .catch(() => {});
  }, []);
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 mb-12">
          <div>
            <div className="flex items-center gap-3 mb-4">
              <div className="w-10 h-10 rounded-lg bg-white/10 border border-white/10 flex items-center justify-center font-display font-bold text-sm text-white">DN</div>
              <div>
                <p className="font-display font-semibold text-white text-[15px]">Dieulin Napoleon</p>
                <p className="text-[10px] font-semibold tracking-[0.12em] uppercase text-gold">{t('hero.tagline')}</p>
              </div>
            </div>
            <p className="text-sm leading-relaxed mt-4">{t('footer.tagline')}</p>
          </div>
          <div>
            <h3 className="font-display font-semibold text-white text-sm mb-4">{t('footer.navigate')}</h3>
            <ul className="space-y-2">
              {FOOTER_NAV_KEYS.map((link) => (
                <li key={link.href}><Link href={link.href} className="text-sm hover:text-gold transition-colors">{t('nav.' + link.key)}</Link></li>
              ))}
            </ul>
          </div>
          <div>
            <h3 className="font-display font-semibold text-white text-sm mb-4">{t('footer.ventures')}</h3>
            <ul className="space-y-2">{VENTURES.map((v) => (<li key={v} className="text-sm">{v}</li>))}</ul>
          </div>
          <div>
            <h3 className="font-display font-semibold text-white text-sm mb-4">{t('sections.connect')}</h3>
            <div className="flex flex-wrap gap-3 mb-4">
              {socials.map((social) => (
                <a key={social.id} href={social.url} target="_blank" rel="noopener noreferrer" className="w-9 h-9 rounded-lg bg-white/5 border border-white/10 flex items-center justify-center hover:bg-gold/20 hover:border-gold/30 hover:text-gold transition-all" aria-label={social.platform}>
